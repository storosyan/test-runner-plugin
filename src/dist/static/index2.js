...
var TestRunnerView = Backbone.Marionette.View.extend({
  ...
  updateCount: function() {
   let allTestTags = this.el.getElementsByClassName("tags");
    let totalSelectedCountsTmp = 0;
    for (let i = 0; i < allTestTags.length; i++) {
      //debugger;
      try{
        let val = parseInt(allTestTags[i].parentElement.children[1].attributes.style.value.lastIndexOf("#97cc64"));
       // console.log(val)
        //debugger;
        if (val > 0) {
          totalSelectedCountsTmp += 1;
         // console.log(allTestTags[i].parentElement.children[1].attributes.style.value.lastIndexOf("#97cc64"))
        }
      } catch(err) {
        //
      }
    }
    console.log(totalSelectedCountsTmp)
    this.el.getElementsByClassName(
      "selectedTestsCount"
    )[0].innerText = totalSelectedCountsTmp;
  },
//Not capturing count while adding color, calculating after color handling.
